local<-"C:\\Users\\Steven\\Desktop\\Microdados\\Microdados_2016"
setwd(local)
getwd()
...
ALUNO<-read.csv2.ffdf(file="DADOS/DM_ALUNO.csv",sep="|",first.rows=1000000)
#É importante que o Pipe( | ) seja utilizado como delimitador.

#---------------------------------------------------------------------
# Somente a UNIRIO
#---------------------------------------------------------------------
ALUNO_UNIRIO16 <- subset(ALUNO , CO_IES == 693)
ALUNO_UNIRIO16 <- data.frame(ALUNO_UNIRIO16)
...
ALUNO_UNIVERSIDADE_RJ_16<- subset(ALUNO , CO_IES == 693 | CO_IES ==  15 | CO_IES == 140 | 
...
ALUNO_UNIVERSIDADE_RJ_16 <- data.frame(ALUNO_UNIVERSIDADE_RJ_16)

saveRDS(ALUNO_UNIVERSIDADE_RJ_16,"ALUNO_UNIVERSIDADE_RJ_16.RDS")
